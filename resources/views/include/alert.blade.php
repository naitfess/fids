...
@php
    $alertClass = session('status', 'info');
    $alertIcon = 'info-fill';
    if ($alertClass === 'success') {
        $alertIcon = 'check-circle-fill';
    } elseif ($alertClass === 'danger') {
        $alertIcon = 'exclamation-triangle-fill';
    }
@endphp

@if (session('message'))
    <div class="alert alert-{{ $alertClass }} alert-dismissible fade show d-flex align-items-center mb-0" role="alert">
